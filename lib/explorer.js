...
Return ONLY a JSON object: {"pageName": "<short screen name>", "controls": ["<element id>", ...], "typeInputs": ["<element id>", ...], "links": ["<same-origin path>", ...], "avoidPaths": ["<path fragment>", ...]}`;
...
${lines.join("\n")}`;
  try {
    ...
    return { pageName: snap.title, controls: [], typeInputs: [], avoidPaths: [], links: snap.elements.filter((e) => e.href?.startsWith("/")).map((e) => e.href).slice(0, 5) };
  }
}
