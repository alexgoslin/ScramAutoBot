...
Links: choose same-origin paths for screens of a DIFFERENT kind than those already visited or queued — one example per template (one profile, one post detail, one settings sub-page, one conversation…). Prefer primary navigation, sidebars, tabs and settings sections. Skip log out, delete, billing/checkout/payment, external sites, and legal/cookie/help boilerplate unless the site is tiny.

Elements marked BLOCKED will not be clicked whatever you choose — don't pick them.

Return ONLY a JSON object: {"pageName": "<short screen name>", "controls": ["<element id>", ...], "typeInputs": ["<element id>", ...], "links": ["<same-origin path>", ...]}`;
...
async function planPage(ctx, snap) {
  ...
  const userContent = `Page: ${snap.url}
Title: ${snap.title}
Screens already visited: ${ex.visited.slice(-40).join(", ") || "(none)"}
Screens already queued: ${ex.queued.slice(-40).join(", ") || "(none)"}
...
${lines.join("\n")}`;
  try {
    const plan = await askJson({ system: MODE === "safe" ? PLANNER_SAFE : PLANNER_FULL, userContent, maxTokens: 1500, model: s.explorerModel });
    ...
    return { pageName: snap.title, controls: [], typeInputs: [], links: snap.elements.filter((e) => e.href?.startsWith("/")).map((e) => e.href).slice(0, 5) };
  }
}
...
export async function explorePage(ctx) {
  ...
  await ctx.log(`“${plan.pageName}”: trying ${Math.min(plan.controls.length, s.maxInteractionsPerPage)} control(s)…`);
  ...
  for (const link of plan.links) {
    try {
      enqueue(ctx, new URL(link, url).href);
    } catch {
      /* bad url */
    }
  }
  ...
  ex.pages.push({ url, name: plan.pageName, specId: spec.id });
  ex.current = null;
  await ctx.save({ explore: ex });
